...
import type { DoaInfoItem } from '@/types/DoaInfo'

const route = useRoute()
const doa = ref<DoaInfoItem[]>([])
const slug = computed(() => route.params.slug as string)
const pageTitle = computed(() => `${makeTitle(slug.value)} - Quranku`)

onMounted(async () => {
  const res = await import(`@/data/doa-harian/${slug.value}.json`)
  doa.value = res.data
})
</script>
